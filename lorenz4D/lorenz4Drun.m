  X=lorenz4D(20/400*1530,5,10e-3);
  fig2 = figure('pos',[5 270 600 200],'color','w');
  pcolor(X(1001:1531,:)); shading interp; caxis([-3 3]);
  title('Lorenz-96 501x40 40 is the space');
  xlabel('Space'); ylabel('Time','rotat',0);
  display(size(X));
  display(size(X(1001:1531,:)));
  csvwrite('output_Lorenz_96.csv',X)
